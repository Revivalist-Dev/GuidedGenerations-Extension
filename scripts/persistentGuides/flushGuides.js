...
const flushGuides = async () => { // Make function async
	...
	try {
		debugLog(`[FlushGuides] Executing script to list injections...`);
		...
		debugLog('[FlushGuides] List injections script result:', result);
		...
		if (result && result.pipe) {
			try {
				...
				debugLog('[FlushGuides] Parsed injection object:', injectionObject);
			} catch (parseError) {
...
:}:}|`;
	...
	try {
		debugLog(`[FlushGuides] Executing flush logic script with labels: "${buttonLabels}"`);
		...
		debugLog('[FlushGuides] Flush logic script executed.');
	} catch (error) {
